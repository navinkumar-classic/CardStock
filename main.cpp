...
int main() {
    ...
    std::ifstream ep("../example/JSON/EventParser/EventParser_1.json");
    json epj = json::parse(ep);



    game.actionHandler.addAction("play_card", [](Player &player, CardZoneMap &cardZoneMap, GameState &gameState) {
                                     std::pair<bool, std::vector<int> > r1 = Action::findHandCardsMatchingCardProperty<
                                         std::string>(player, "hand", cardZoneMap.getZone("deckOpen")->peekFront(),
                                                      "color");
                                     std::pair<bool, std::vector<int> > r2 = Action::findHandCardsMatchingCardProperty<
                                         int>(player, "hand", cardZoneMap.getZone("deckOpen")->peekFront(), "number");

                                     return Action::mergeResultsOr({r1, r2});
                                 }, [](PlayerList &playerList, CardZoneMap &cardZoneMap, GameState &gameState,
                                       int cardId) {
                                     Action::playCard(playerList.getCurrentPlayer(), cardZoneMap, "hand", "deckOpen",
                                                      cardId);
                                     return true;
                                 });
    ...
    game.actionHandler.addAction("draw_card", [](Player &player, CardZoneMap &cardZoneMap, GameState &gameState) {
                                     return Action::checkAndTogglePlayerStateIfEqual(player, "drawn", true);
                                       ...
                                       int cardId) {
                                     Action::drawCardFromDeck(playerList.getCurrentPlayer(), cardZoneMap, "hand",
                                                              "deckClose", 1);
    ...
    game.eventsParser.parseEvent(game.eventManager, epj);
